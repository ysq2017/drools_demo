...
- package
    ...
    `package`表示一个命名空间.package是必须定义的，必须放在规则文件第一行. 但对名称没有要求，可以任意写。
- import

    `import`语句的工作方式类似于Java中的import语句。您需要为要在规则中使用的任何对象指定完全限定路径和类型名称。
    可以导入自定义类或其他第三方类（项目已引入）。
- global
...
- function
    ...
    以function开头，语法规则与java类似，不支持重载和可变参数。
    ```
...
#### 属性部分-Attributes
  ...
  - 常用属性 `activation-group`  `date-effective` `enabled` `lock-on-active` `no-loop` `salience`
  ...
  - no-loop
      ...
      只能限制一条规则内的Fact对象的更新后是否循环调用，即此规则的RHS部分，无法阻止其他规则的触发。
  ...
  - lock-on-active
      ...
      限制级别更高，设置true后只能执行一次，可以对单一规则或规则组使用。
